...
tools: [Agent, Read, Write, Edit, Bash, Glob, Grep, TodoWrite, WebFetch, WebSearch]
...
> **Rule:** Implementation MUST NOT start without a complete design paper trail.
> The pipeline value is the audit chain: SRS → BD → Spec → Plan → DD → Tasks → Code.
> Skipping any step breaks traceability and makes the code unreviewable.
...
> **All protocol files live under `.claude/agents/protocols/`.**
...
> **All step files live under `.claude/agents/steps/`.**
