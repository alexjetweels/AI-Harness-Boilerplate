...
- **Output:** specification (`docs/output/specs/<feature-id>/spec.md`), quality checklist (`docs/output/specs/<feature-id>/checklists/requirements.md`)
...
2. **Check for existing spec folder FIRST — prevents duplicate creation**:
   ...
   b. Scan the `docs/output/specs/` directory for an existing folder matching the short-name:
      ```powershell
      Get-ChildItem docs/output/specs/ -Directory | Where-Object { $_.Name -match '<short-name>' }
      ```

   c. **If a matching folder is found** (e.g., `docs/output/specs/001-xxx/spec.md` exists):
      - **Do NOT create a new branch or a new numbered folder.**
      - Set `<feature-id>` = the existing folder name (e.g., `001-xxx`)
      - Set `FEATURE_DIR` = `docs/output/specs/<feature-id>`
      - Set `SPEC_FILE` = `docs/output/specs/<feature-id>/spec.md`
      - **Skip steps 2d (the create-new-feature.ps1 script) entirely** — the folder already exists
      - Log in the phase report: "Detected existing spec folder: `docs/output/specs/<feature-id>/` — skip new creation and update the existing spec instead"
   ...
   e. Find the highest feature number across all sources for the short-name:
      ...
      - Specs directories: Check for directories matching `docs/output/specs/[0-9]+-<short-name>`
...
module-id: <mod-id>
artifacts:
  spec: docs/output/specs/<feature-id>/spec.md
...
critical-issues: []
next-inputs:
  spec-path: docs/output/specs/<feature-id>/spec.md
/STEP-RESULT -->
```
