...
export const PHASE_META = {
  ...
  'H1-context': {
    label: 'Context Build',
    layer: 'H1',
    desc: 'Assemble deterministic context packet before any agent call',
    Icon: Database,
  },
  'H4-context-security': {
    label: 'Context Security',
    layer: 'H4',
    desc: 'Scan context payload for secrets & unsafe patterns',
    Icon: Shield,
  },
  'H4-generated-security': {
    label: 'Generated Security',
    layer: 'H4',
    desc: 'Scan generated artifacts and source for secrets & OWASP issues',
    Icon: Shield,
  },
  // ── IPA Doc Gen phases ──────────────────────────────────────────────────
  'system-srs': {
    label: 'System SRS',
    layer: 'SDLC',
    desc: 'Generate full system-wide requirements specification (okr.srsallsystem)',
    Icon: FileText,
  },
  srs: {
    label: 'SRS',
    layer: 'SDLC',
    desc: 'Software requirements specification per module (okr.srs)',
    Icon: FileText,
  },
  'basic-design': {
    label: 'Basic Design',
    layer: 'SDLC',
    desc: 'External design — high-level architecture decisions (okr.bd)',
    Icon: Layers,
  },
  'detail-design': {
    label: 'Detail Design',
    layer: 'SDLC',
    desc: 'Internal design — component-level documentation (okr.dd)',
    Icon: FileText,
  },
  // ── Spec-Kit phases ──────────────────────────────────────────────────────
  specify: {
    label: 'Spec Creation',
    layer: 'SDLC',
    desc: 'Feature spec via Spec-Kit (speckit.specify → spec.md)',
    Icon: Code2,
  },
  clarify: {
    label: 'Clarify',
    layer: 'SDLC',
    desc: 'Resolve all [NEEDS CLARIFICATION] markers autonomously',
    Icon: Search,
  },
  plan: {
    label: 'Plan',
    layer: 'SDLC',
    desc: 'Implementation plan + data-model + contracts (speckit.plan)',
    Icon: Layers,
  },
  tasks: {
    label: 'Task Breakdown',
    layer: 'SDLC',
    desc: 'Decompose plan into discrete executable tasks (speckit.tasks)',
    Icon: Activity,
  },
  implement: {
    label: 'Implementation',
    layer: 'SDLC',
    desc: 'AI-driven code generation with auto build & fix (speckit.implement)',
    Icon: Code2,
  },
  // ── Review phases ────────────────────────────────────────────────────────
  'review-spec': {
    label: 'Spec Review',
    layer: 'SDLC',
    desc: 'Validate spec.md quality & completeness (okr.reviewspec)',
    Icon: Search,
  },
  'review-plan': {
    label: 'Plan Review',
    layer: 'SDLC',
    desc: 'Validate plan.md conformance to spec (okr.reviewplan)',
    Icon: Search,
  },
  'review-code': {
    label: 'Code Review',
    layer: 'SDLC',
    desc: 'Code quality, DB data check, security (okr.reviewcode)',
    Icon: Search,
  },
  // ── Test phases ───────────────────────────────────────────────────────────
  'generate-tests': {
    ...
    layer: 'SDLC',
    desc: 'Generate test cases from SRS/BD/DD (okr.testkit gen-testcases)',
    Icon: TestTube2,
  },
  'run-tests': {
    label: 'Run Tests',
    layer: 'SDLC',
    desc: 'Execute automated tests — BACK-TO-PLAN on fail (okr.testkit run-tests)',
    Icon: TestTube2,
  },
  // ── Final phase ───────────────────────────────────────────────────────────
  'verify-launch': {
    label: 'Verify & Launch',
    layer: 'H3',
    desc: 'Final acceptance gate: build, test, docker compose up',
    Icon: CheckCircle2,
  },
  // ── Boss-mode phase ───────────────────────────────────────────────────────
  'boss-pipeline': {
    label: 'Boss Pipeline',
    layer: 'SDLC',
    desc: 'Full 13-step orchestration via okr.bossbuiltin (boss mode)',
    Icon: Activity,
  },
  ...
  analyze: {
    label: 'Analysis',
    layer: 'SDLC',
    desc: 'Cross-artifact consistency and dependency check',
    Icon: Search,
  },
  test: {
    label: 'Testing',
    layer: 'SDLC',
    desc: 'Test case generation and automated validation',
    Icon: TestTube2,
  },
  review: {
    label: 'Code Review',
    layer: 'SDLC',
    desc: 'Automated quality, logic, and standards review',
    Icon: Search,
  },
  'security-review': {
    label: 'Security Review',
    layer: 'H4',
    desc: 'Security vulnerability and OWASP audit',
    Icon: Shield,
  },
  verify: {
    label: 'Verify',
    layer: 'H3',
    desc: 'Final acceptance gate — deterministic pass/fail check',
    Icon: CheckCircle2,
  },
};
...
export const LAYER_STYLE = {
  ...
  SDLC: {
    color: '#65c6a5',
    bg: 'rgba(101,198,165,0.1)',
    border: 'rgba(101,198,165,0.3)',
  },
};
...
export function phaseRunStatus(phaseData, phaseName, currentPhase, runStatus) {
  if (!phaseData) {
    return phaseName === currentPhase && runStatus === 'running'
      ? 'running'
      : 'pending';
  }
  if (phaseData.status === 'done')
    return phaseData.gate === 'fail' ? 'done-fail' : 'done';
...
export function useLogTail(runId) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/harness-runs/${runId}/log?lines=300`,
      );
      if (!res.ok) return;
      const data = await res.json();
...
const PhaseNodeInner = ({ data }) => {
  const { index, phaseName, phaseData, status, timelineEntry, isSelected } =
    data;
  ...
  } else if (isDoneFail || isFailed) {
    statusColor = '#ffb39f';
    StatusIcon = <AlertCircle size={13} />;
  } else if (isRunning) {
    statusColor = '#65c6a5';
    StatusIcon = <Activity size={13} className='spin' />;
  }
  ...
  return (
    ...
    >
      ...
      <div className='pfn-header'>
        ...
        <span className='pfn-status-icon' style={{ color: statusColor }}>
          {StatusIcon}
        </span>
      </div>
      ...
      {hasMetrics && (
        <div className='pfn-metrics'>
          ...
          {timelineEntry?.duration_sec != null && (
            <span>{timelineEntry.duration_sec}s</span>
          )}
          {timelineEntry?.cost_usd > 0 && (
            <span>${Number(timelineEntry.cost_usd).toFixed(4)}</span>
          )}
          {phaseData.failed_gates?.length > 0 && (
            <span className='pfn-metric-fail'>
              {phaseData.failed_gates.length} gate
              {phaseData.failed_gates.length > 1 ? 's' : ''} failed
            </span>
          )}
        </div>
      )}
      ...
      <Panel position='top-right'>
        <div className='flowLegend'>
          ...
          <span className='legend-item'>
            <span className='legend-dot' style={{ background: '#8dcc61' }} />
            Done
          </span>
          <span className='legend-item'>
            <span
              className='legend-dot pulse-dot'
              style={{ background: '#65c6a5' }}
            />
            Running
          </span>
          <span className='legend-item'>
            <span className='legend-dot' style={{ background: '#ffb39f' }} />
            Failed
          </span>
        </div>
      </Panel>
...
function ArtifactItem({ artifact, runId }) {
  ...
  async function toggle() {
    ...
    if (content === null) {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE}/api/harness-runs/${runId}/artifacts/${artifactId}`,
        );
  ...
  return (
    <div
      className={`dp-artifact dp-artifact--btn${expanded ? ' dp-artifact--open' : ''}`}
    >
      <button
      ...
      >
        ...
        {loading ? (
          ...
          <ChevronDown
            size={11}
            className={`dp-artifact-chevron${expanded ? ' dp-artifact-chevron--open' : ''}`}
          />
        )}
      </button>
      {expanded && (
...
export function PhaseDetailPanel({
  ...
  const timelineEntry = phaseTimeline.find(
    (t) => t.phase_name === selectedPhase,
  );

  const phaseGates = gatesByPhase[selectedPhase];
  const latestAttempt = phaseGates
    ? Math.max(...Object.keys(phaseGates).map(Number))
    : null;
  const gateOutcomes =
    latestAttempt != null
      ? phaseGates[String(latestAttempt)] || phaseGates[latestAttempt] || []
      : [];
  ...
  const artifacts = (run?.artifacts || []).filter((a) => {
    if (selectedPhase === 'H1-context') {
      return (
        a.artifact_type === 'context_packet' ||
  ...
  const runStatus = phaseRunStatus(
    phaseData,
    selectedPhase,
    run?.current_phase,
    run?.status,
    ...
    <aside className='detailPanel'>
      ...
      <div className='dp-metrics'>
        <div className='dp-metric'>
          <span>Status</span>
          <strong className={`dp-status-val ${runStatus}`}>
            {statusLabel(phaseData?.status || 'pending')}
          </strong>
        </div>
        <div className='dp-metric'>
          <span>Gate</span>
          <strong className={`dp-gate-val ${phaseData?.gate || ''}`}>
            {phaseData?.gate || '—'}
          </strong>
        </div>
        <div className='dp-metric'>
        ...
        <div className='dp-metric'>
          ...
          <strong>
            {timelineEntry?.cost_usd > 0
              ? `$${Number(timelineEntry.cost_usd).toFixed(4)}`
              : '—'}
          </strong>
        </div>
        <div className='dp-metric'>
          ...
          <strong
            className={
              timelineEntry?.agent_ok === true
                ? 'dp-status-val done'
                : timelineEntry?.agent_ok === false
                  ? 'dp-status-val failed'
                  : ''
            }
          >
            {timelineEntry?.agent_ok === true
      ...
      {gateOutcomes.length > 0 && (
        <div className='dp-section'>
          <h4 className='dp-section-title'>
            Gate Checks · attempt {latestAttempt}
          </h4>
            ...
            {gateOutcomes.map((g, i) => (
              <div key={i} className={`dp-gate ${g.passed ? 'pass' : 'fail'}`}>
                ...
                <div className='dp-gate-body'>
                  ...
                  {!g.passed && g.report && (
                    <p className='dp-gate-report'>{g.report}</p>
                  )}
                </div>
              </div>
            ))}
      ...
      {!gateOutcomes.length && phaseData?.failed_gates?.length > 0 && (
        <div className='dp-section'>
          ...
          <div className='dp-failed-tags'>
            {phaseData.failed_gates.map((g) => (
              <span key={g} className='gateTag'>
                {g}
              </span>
            ))}
          </div>
      ...
      {artifacts.length > 0 && (
        <div className='dp-section'>
          <h4 className='dp-section-title'>
            ...
            Artifacts{' '}
            <small
              style={{ fontWeight: 400, color: 'var(--c-on-surface-muted)' }}
            >
              — click to expand
            </small>
          </h4>
    ...
    <aside className='detailPanel'>
        ...
        <div className={`escalationBanner ${run.status}`}>
          ...
          <div>
            ...
            {failedPhases.length > 0 && (
              <span>
                {' '}
                — failed at: {failedPhases.map((p) => p.name).join(', ')}
              </span>
            )}
          </div>
        </div>
      ...
      <div className='dp-metrics'>
        ...
        <div className='dp-metric'>
          ...
          <strong>${Number(run?.cost_usd || 0).toFixed(4)}</strong>
        </div>
        <div className='dp-metric'>
          <span>Return</span>
          <strong>{run?.return_code ?? '—'}</strong>
        </div>
        <div className='dp-metric'>
          <span>Phases</span>
          <strong>
            {run?.phases
              ? `${run.phases.filter((p) => p.status === 'done').length} / ${run.phases.length}`
              : '—'}
          </strong>
        </div>
      </div>
      ...
      <div className='dp-section dp-section-grow'>
        <div className='dp-tabs'>
          ...
          >
            ...
            Events
            {events.length > 0 && (
              <span className='dp-tab-badge'>{events.length}</span>
            )}
          </button>
          <button
          ...
          >
            ...
            System Log
            {logLines.length > 0 && (
              <span className='dp-tab-badge'>{logLines.length}</span>
            )}
          </button>
        </div>
        ...
        {tab === 'log' && (
          <div className='dp-event-log dp-event-log-fill dp-raw-log'>
            {logLines.length === 0 ? (
              <div className='dp-empty-log'>
                No system log yet — log file written when run starts.
              </div>
            ) : (
              logLines.map((line, i) => (
                <div key={i} className='rawLogLine'>
                  {line || ' '}
                </div>
              ))
            )}
